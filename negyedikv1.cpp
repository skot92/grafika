...
void initFaces() {
	...
	for (int i = 0; i < PIdb; i++) {
		for (int j = 0; j < PIdb; j++) {
			faces[db] = initFace((i + 1) * (PIdb + 1) + j, (i)* (PIdb + 1) + j, (i)* (PIdb + 1) + (j + 1), (i + 1) * (PIdb + 1) + (j + 1));
			++db;
		}
	}
}
...
int comparePointsZ(const void *a, const void *b) {
	if ( (*(FACE*)a).v[0] < (*(FACE*)b).v[0]) return -1;
	if ( (*(FACE*)a).v == (*(FACE*)b).v) return  0;
	if ( (*(FACE*)a).v > (*(FACE*)b).v) return  1;
}
...
*/
/*
void Paint(projectedFaces, context) {
	qsort(identityCube, 64, sizeof(VECTOR3), comparePointsZ)
